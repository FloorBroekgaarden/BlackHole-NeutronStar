...
def returnSeedsPercentageDCO(pathCOMPASOutput='/Volumes/Andromeda/DATA/AllDCO_bugfix/fiducial/',\
    ...
    DataNotInHubble = Data
    DataNotInHubble.setCOMPASDCOmask(types=types,  withinHubbleTime=False, optimistic=optimistic)
    DataNotInHubble.setCOMPASData()
    SeedsDCOnotInThubble    = DataNotInHubble.seeds



    Data.setCOMPASDCOmask(types=types,  withinHubbleTime=True, optimistic=optimistic)
    Data.setCOMPASData()
    SeedsDCO    = Data.seeds

    # get rid of the DCOs that merge in a Hubble time, we only want the ones exceeding hubble time
    mask_notInHubble= np.ones_like(SeedsDCOnotInThubble)
    print(mask_notInHubble)
    print(np.asarray(SeedsDCOnotInThubble))
    print(np.asarray(SeedsDCO))
    mask_notInHubble -= np.in1d(np.asarray(SeedsDCOnotInThubble), np.asarray(SeedsDCO))
    maskk = (mask_notInHubble==1)
    SeedsDCOnotInThubble = SeedsDCOnotInThubble[maskk]
    
    allSeedsSystems  = f['systems']['SEED'][...].squeeze()
    weights    = f['systems']['weight'][...].squeeze()

    maskSystemsDCOnotInThubble = np.in1d(allSeedsSystems, SeedsDCOnotInThubble)
    maskSystemsDCO = np.in1d(allSeedsSystems, SeedsDCO)
    # seedsSystemBHNS = allSeedsSystems[maskSystemsBHNS]


    normalisation = float(np.sum(weights)) # sum all weights of all systems. 
    percentageDCOnotInThubble = np.sum(weights[maskSystemsDCOnotInThubble]) / normalisation 
    percentageDCO = np.sum(weights[maskSystemsDCO]) / normalisation # of all systems. 


    
    return SeedsDCO, SeedsDCOnotInThubble, percentageDCO, percentageDCOnotInThubble



def returnSeedsPercentagedisruptSNe(pathCOMPASOutput='/Volumes/Andromeda/DATA/AllDCO_bugfix/fiducial/'):
    ...
    # nr systems that disrupt during 1st SN
    primary   = f['supernovae']['whichStar'][...].squeeze() ==1
    survived  = f['supernovae']['Survived'][...].squeeze() == False 
    maskInterest =  primary & survived

    seedsSN1disrupt  = f['supernovae']['randomSeed'][...].squeeze()[maskInterest]
    maskSeedsOfInterest = np.in1d(allSeedsSystems, seedsSN1disrupt)
    percentageSN1disrupt = np.sum(weights[maskSeedsOfInterest]) / normalisation



    # nr systems that disrupt during 2nd SN
    secondary   = f['supernovae']['whichStar'][...].squeeze() ==2
    maskInterest =  secondary & survived

    seedsSN2disrupt  = f['supernovae']['randomSeed'][...].squeeze()[maskInterest]
    maskSeedsOfInterest = np.in1d(allSeedsSystems, seedsSN2disrupt)
    percentageSN2disrupt = np.sum(weights[maskSeedsOfInterest]) / normalisation



    
    return seedsSN1disrupt, seedsSN2disrupt, percentageSN1disrupt, percentageSN2disrupt




def returnSeedsPercentagedisruptStellarMerger(pathCOMPASOutput='/Volumes/Andromeda/DATA/AllDCO_bugfix/fiducial/'):
    ...
    # nr systems that disrupt during 1st SN
    stellar_merger  = f['systems']['stellar_merger'][...].squeeze() ==1
   
    stellar_merger = np.asarray(stellar_merger)

    seedsStellar_merger  = allSeedsSystems[stellar_merger]

    percentageStellar_merger = np.sum(weights[stellar_merger]) / normalisation



    
    return seedsStellar_merger, percentageStellar_merger
...
def returnSeedsPercentageOther(pathCOMPASOutput='/Volumes/Andromeda/DATA/AllDCO_bugfix/fiducial/',channelsSeedsList=[]):
    ...
    mask_notInChannels = np.ones_like(allSeedsSystems)
    for seeds in channelsSeedsList:
        mask_notInChannels -= np.in1d(allSeedsSystems, np.array(seeds))

    maskk = (mask_notInChannels==1)

    seedsChannel = allSeedsSystems[maskk]
    percentageChannel = (np.sum(weights[maskk])/normalisation)
